...
describe("Company.findAll", () => {
  ...
  test("returns companies with minimum employees specified", async () => {
  ...
  test("returns companies with maximum employees specified", async () => {
  ...
  test("returns companies with specified range of employees", async () => {
    const companies = await Company.findAll({ minEmployees: 1, maxEmployees: 1 });
    expect(companies).toEqual([
      { handle: "c1", name: "C1", description: "Desc1", numEmployees: 1, logoUrl: "http://c1.img" }
    ]);
  });

  test("returns companies with specified name", async () => {
  ...
  test("returns empty list when no companies found with specified name", async () => {
    const companies = await Company.findAll({ name: "nope" });
    expect(companies).toEqual([]);
  });

  test("throws BadRequestError when minEmployees is greater than maxEmployees", async () => {
    await expect(Company.findAll({ minEmployees: 10, maxEmployees: 1 })).rejects.toThrow(BadRequestError);
  });
});
