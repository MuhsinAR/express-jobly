...
describe("User.applyToJob", () => {
  ...
  test("not found if no such job", async () => {
    await expect(User.applyToJob("u1", 0, "applied")).rejects.toThrow(
      NotFoundError
    );
  });

  test("not found if no such user", async () => {
    await expect(User.applyToJob("nope", testJobIds[0], "applied")).rejects.toThrow(
      NotFoundError
    );
  });
});
